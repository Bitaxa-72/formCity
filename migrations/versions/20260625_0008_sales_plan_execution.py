...
def upgrade() -> None:
    ...
    for column in ("project", "snapshot_month", "file_hash"):
        op.create_index(f"ix_sales_plan_execution_sources_{column}", "sales_plan_execution_sources", [column])
    ...
    op.create_table(
        ...
        sa.Column("block_label", sa.String(length=255), nullable=False),
    ...
    for column in (
        ...
        op.create_index(f"ix_sales_plan_execution_facts_{column}", "sales_plan_execution_facts", [column])
...
def downgrade() -> None:
    for column in reversed(
        ...
        op.drop_index(f"ix_sales_plan_execution_facts_{column}", table_name="sales_plan_execution_facts")
    op.drop_table("sales_plan_execution_facts")

    for column in reversed(("project", "snapshot_month", "file_hash")):
        op.drop_index(f"ix_sales_plan_execution_sources_{column}", table_name="sales_plan_execution_sources")
    op.drop_table("sales_plan_execution_sources")
