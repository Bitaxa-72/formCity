...
- `summary_rows` - безопасные строки сводных таблиц без запрещенных колонок;
- `summary_search` - поиск по безопасным строкам и ячейкам сводных таблиц.
...
В сводных таблицах по умолчанию выводится вся безопасная информация: метрики, числовые значения, статусы, даты, площади, цены, суммы, типы помещений, корпуса, этажи, номера помещений и другие незапрещенные поля.

Импортер помечает чувствительные ячейки через `is_sensitive = true` только для закрытого списка колонок:
- ФИО клиента;
...
Запрещенные ячейки не выводятся в Telegram напрямую. Все остальные поля участвуют в выдаче, поиске, фильтрах, группировках и подробных строках.
