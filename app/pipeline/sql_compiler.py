...
def compile_model_raw_rows_sql(frame: QueryFrame) -> SQLQuery:
    ...
    return SQLQuery(
        sql=(
            ...
            "  GROUP_CONCAT(\n"
            "    c.column_letter || ': ' || COALESCE(CASE WHEN c.value_text IS NOT NULL AND c.value_text NOT LIKE '%-%-%' AND c.value_text GLOB '*[0-9]*' AND c.value_text NOT GLOB '*[^0-9., -]*' THEN printf('%.6f', CAST(REPLACE(REPLACE(c.value_text, ' ', ''), ',', '.') AS REAL)) ELSE c.value_text END, CASE WHEN c.value_number IS NOT NULL THEN printf('%.6f', c.value_number) END, c.value_date, CAST(c.value_bool AS TEXT)),\n"
            "    ' | '\n"
            "  ) AS values_preview,\n"
            "  GROUP_CONCAT(\n"
            "    c.column_letter || ': ' || COALESCE((\n"
            "      SELECT hc.value_text\n"
            "      FROM model_raw_cells hc\n"
            "      WHERE hc.project = r.project\n"
            "        AND hc.snapshot_month = r.snapshot_month\n"
            "        AND hc.source_file = r.source_file\n"
            "        AND hc.sheet_name = r.sheet_name\n"
            "        AND hc.column_number = c.column_number\n"
...
def compile_summary_raw_rows_sql(frame: QueryFrame) -> SQLQuery:
    ...
    return SQLQuery(
        sql=(
            ...
            "  COUNT(c.id) AS visible_cells,\n"
            "  GROUP_CONCAT(\n"
            "    COALESCE(c.header_label, c.column_letter) || ': ' || COALESCE(CASE WHEN c.value_text IS NOT NULL AND c.value_text NOT LIKE '%-%-%' AND c.value_text GLOB '*[0-9]*' AND c.value_text NOT GLOB '*[^0-9., -]*' THEN printf('%.6f', CAST(REPLACE(REPLACE(c.value_text, ' ', ''), ',', '.') AS REAL)) ELSE c.value_text END, CASE WHEN c.value_number IS NOT NULL THEN printf('%.6f', c.value_number) END, c.value_date, CAST(c.value_bool AS TEXT)),\n"
            "    ' | '\n"
            "  ) AS values_preview\n"
            "FROM summary_rows r\n"
